...
class Chat implements MessageComponentInterface {
    ...
    public function onMessage(ConnectionInterface $from, $msg) {
        ...
        if (isset($data['command'])) {
            if ($data['command'] == 'private') {
                $this->handlePrivateChat($from, $data);
            } else {
                $this->handleGroupChat($from, $data);
            }
        } else {
            // Fallback behavior: assume it's a group chat if 'command' is missing
            $this->handleGroupChat($from, $data);
        }
    }

    private function handlePrivateChat(ConnectionInterface $from, $data) {
    ...
    private function handleGroupChat(ConnectionInterface $from, $data) {
